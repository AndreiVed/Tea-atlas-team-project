...
class TeaViewSetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.country1 = Country.objects.create(name="China")
        self.country2 = Country.objects.create(name="Japan")
        self.region1 = Region.objects.create(country=self.country1, province="Yunnan")
        self.region2 = Region.objects.create(country=self.country2, province="Kioto")
        self.category1 = Category.objects.create(name="Pu-erh", region=self.region1)
        self.category2 = Category.objects.create(name="Sencha", region=self.region2)
        ...
        self.tea3 = Tea.objects.create(
            name="Sencha",
            description="Fresh japan green tea",
            category=self.category2,
            impact="Cooling",
        )
        self.tea3.descriptors.add(self.descriptor2)
        self.tea3.descriptors.add(self.descriptor1)
    ...
    def test_filter_by_one_category(self):
    ...
    def test_filter_by_two_categories(self):
        """Перевірка фільтрації за категорією"""
        response = self.client.get(TEA_URL, {"type": "Pu-erh,Sencha"})

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 3)

    def test_filter_by_one_country(self):
    ...
    def test_filter_by_two_countries(self):
        """Перевірка фільтрації за країною"""
        response = self.client.get(TEA_URL, {"country": "China,Japan"})

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 3)

    def test_filter_by_one_impact(self):
    ...
    def test_filter_by_two_impacts(self):
        """Перевірка фільтрації за впливом"""
        response = self.client.get(TEA_URL, {"impact": "Warming,Cooling"})
