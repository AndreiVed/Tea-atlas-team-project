...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const redirectToFrontend = setTimeout(
    ...
    fetch(backendLoginUrl, {
      ...
      .then((response) => {
        ...
        if (!response.ok) {
          return response.json().then((err) => {
            ...
            throw new Error(err.error || "Помилка автентифікації на бекенді");
          });
        }
      ...
      .then((data) => {
        ...
        if (data && data.access && data.refresh) {
          ...
          alert("Помилка: Не отримано токени автентифікації від бекенду.");
          redirectToFrontend;
        }
      })
      .catch((error) => {
